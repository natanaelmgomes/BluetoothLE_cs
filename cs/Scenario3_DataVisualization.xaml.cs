...
namespace SDKTemplate
{
    ...
    public sealed partial class Scenario3_DataVisualization : Page
    {
        ...
        #region UI Code
        public Scenario3_DataVisualization()
        {
            InitializeComponent();
        }
        ...
        private string FormatValueByPresentation(IBuffer buffer, GattPresentationFormat format)
        {
            ...
            if (format != null)
            {
                Debug.WriteLine("Format is not null");
                ...
                else if (format.FormatType == GattPresentationFormatTypes.UInt16)
                {
                    try
                    {
                        return BitConverter.ToUInt16(data, 0).ToString(); // uint16_t
                    }
                    catch (ArgumentException)
                    {
                        return "(error: Invalid UInt16 Format)";
                    }
                }
                else
                {
            ...
            else if (data != null)
            {
                ...
                {
                    Debug.WriteLine("Battery level GATT, from UUID");
                    try
                    {
            ...
            return "Completely unknown format";
        }
